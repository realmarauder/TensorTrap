...
class BaseExternalValidator(ABC):
    ...
    def _unavailable_result(self) -> ExternalValidationResult:
        ...
        hint = install_hints.get(self.required_tool, f"install {self.required_tool}")
...
class ExiftoolValidator(BaseExternalValidator):
    ...
    TEXT_FIELDS = [
        ...
        "Caption-Abstract", "Copyright", "Artist", "Author", "Creator",
        "Software", "XPComment", "XPTitle", "Title", "Subject", "Keywords",
        "Instructions", "SpecialInstructions", "DocumentNotes",
    ]
...
class BinwalkValidator(BaseExternalValidator):
    ...
    ARCHIVE_SIGNATURES = [
        ...
        "tar archive", "cab archive",
    ]
    
    # False positive signatures
    FALSE_POSITIVES = [
        "stuffit", "bix header", "qualcomm", "intel x86",
        "device tree", "copyright", "boot sector",
    ]
    ...
    def validate(
        ...
        if extraction["success"]:
            ...
            return ExternalValidationResult(
                ...
                tool_name=self.name,
                tool_available=True,
                details=f"Archive signature found but extraction failed: {extraction.get('error', 'unknown')}",
...
class ExternalValidationRunner:
    ...
    def validate_finding(
        ...
        offset: Optional[int] = None,
    ) -> Optional[ExternalValidationResult]:
        """
            ...
            if result:
                ...
                if result.status == ExternalValidationStatus.NOT_CONFIRMED:
                    ...
                    if "adjusted_severity" in finding_copy:
                        ...
                        finding_copy["adjusted_severity"] = orig.replace("-HIGH", "-LOW").replace("-MEDIUM", "-LOW")
